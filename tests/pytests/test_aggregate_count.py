...
def test_cursor(protocol):
    env = Env(protocol=protocol)
    docs = 2265
    _setup_index_and_data(env, docs)
    query = ['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'LIMIT', 0, 50, 'WITHCURSOR', 'COUNT', 10]
    res, cursor = env.cmd(*query)
    print(res)
...
def test_cursor_resp2():
    test_cursor(2)

def test_cursor_resp3():
    test_cursor(3)
...
def _test_ftsearch(protocol):
    ...
    queries_and_results = [
        ...
        # # length of results = min(total_results, LIMIT)
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'LIMIT', 0, 50], docs, 50),
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'LIMIT', 0, int(docs/2)], int(docs/2), int(docs/2)),
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'LIMIT', 0, docs*4], docs, docs),
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'LIMIT', 10, 50], 50, 50),
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'LIMIT', 100, docs], docs - 100, docs - 100),
        # # (['FT.AGGREGATE', 'idx', '@price:[1, 100]', 'WITHCOUNT', 'LIMIT', 0, docs], 100, 100),
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'LIMIT', docs, docs*2], 0, 0), # total_results = 0 or docs?
        ...
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'SORTBY', 1, '@title', 'LIMIT', 0, int(docs/2)], docs, int(docs/2)),
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'SORTBY', 1, '@title', 'LIMIT', 0, docs*4], docs, docs),
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'SORTBY', 1, '@title', 'LIMIT', 10, 50], 50, 50),
        # (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'SORTBY', 1, '@title', 'LIMIT', 100, docs], docs - 100, docs - 100),
        # (['FT.AGGREGATE', 'idx', '@price:[1, 100]', 'WITHCOUNT', 'SORTBY', 1, '@title', 'LIMIT', 0, docs], 100, 100),

        # WITHCOUNT + LOAD
        (['FT.AGGREGATE', 'idx', '*', 'WITHCOUNT', 'LOAD', 1, '@title'], docs, docs),

        # (['FT.AGGREGATE', 'idx', '*', 'WITHOUTCOUNT', 'SORTBY', '1', '@title'], docs, 10),
    ]
    for query, expected_total_results, expected_results in queries_and_results:
        profile = ['FT.PROFILE']
        profile.append(query[1]) # index name
        ...
        for command in [query, profile]:
            ...
            for dialect in [2]:
                ...
                if command[0] in ['FT.SEARCH', 'FT.AGGREGATE']:
                    total_results = _get_total_results(res)
                    results = _get_results(res)
                    print(f'total_results: {total_results}   (expected: {expected_total_results})')
                    print(f'len(results): {len(results)}   (expected: {expected_results})')
                else:
                    if protocol == 3:
                        if env.isCluster():
                            print('Cluster')
                            if query[0] == 'FT.SEARCH':
                                print(res['Profile']['Shards'][0]['Result processors profile'])
                                print(res['Profile']['Coordinator']['Result processors profile'])
                            elif query[0] == 'FT.AGGREGATE':
                                for i, shard in enumerate(res['Profile']['Shards']):
                                    print(f'Shard[{i}]')
                                    print(shard['Result processors profile'])
                                print('Coordinator')
                                print(res['Profile']['Coordinator']['Result processors profile'])
                        else:
                            print('Standalone')
                            if query[0] == 'FT.SEARCH':
                                ...
                                profile = res['Profile']['Shards'][0]['Result processors profile']
                                RP_list = [item['Type'] for item in profile]
                                print(f'Shard[0]: {RP_list}')
                                print(profile)
                    elif protocol == 2:
                        if env.isCluster():
                            print('Cluster')
                            for i in [0, 1, 2]:
                                print()
                                profile = res[1][1][i][13]
                                RP_list = [item[1] for item in profile]
                                print(f'Shard[{i}]: {RP_list}')
                                print(profile)
                            coordinator = res[1][3][11]
                            RP_list = [item[1] for item in coordinator]
                            print(f'Coordinator: {RP_list}')
                            print(coordinator)
                        else:
                            print('Standalone')
                            profile = res[1][1][0][13]
