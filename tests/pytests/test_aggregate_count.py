...
def _test_withcount(protocol):
    ...
    if env.isCluster():
        print(f'cluster RESP{protocol}')
    else:
        print(f'standalone RESP{protocol}')
...
def _test_ftsearch(protocol):
    ...
    queries_and_results = [
        ...
        # WITHCOUNT + LIMIT 0 0
        # (['FT.AGGREGATE', 'games', '*', 'WITHCOUNT', 'LIMIT', 0, 0], docs, 0),
        ...
        # # (['FT.AGGREGATE', 'games', '@price:[1, 100]', 'WITHCOUNT', 'LIMIT', 0, docs], 100, 100),
        # (['FT.AGGREGATE', 'games', '*', 'WITHCOUNT', 'LIMIT', docs, docs*2], 0, 0), # total_results = 0 or docs?
        ...
        # (['FT.AGGREGATE', 'games', '*', 'WITHOUTCOUNT', 'SORTBY', 1, '@title', 'LIMIT', 0, 50], docs, 50),
    ]
    for query, expected_total_results, expected_results in queries_and_results:
        profile = ['FT.PROFILE']
        profile.append(query[1]) # index name
