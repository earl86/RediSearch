...
pub struct Optional<'index, I> {
    ...
    /// By default this is `None`, only once we had to "shelve"
    /// the result of the `child` iterator during a call to `RQEIterator::read`,
    /// it will be assigned the doc id of the "current" result if and only if
    /// _that_ id is not sequential based on the current id known.
    ///
    /// Why? Well, optional iterator guarantees to return read calls
    /// to return sequential results by id, filling the gaps with virtual
        ...
        let maybe_real = self
            ...
            .map(|child| {
                if let Some(at_doc_id) = self.child_result_shelved {
                    if self.result.doc_id + 1 == at_doc_id && child.last_doc_id() == at_doc_id {
                        self.child_result_shelved = None;
                        return Ok(child.current());
                    } else if self.result.doc_id >= at_doc_id {
                        // reset it as we must have skipped past it in
                        // a previous skip_to call... This reset ensures we do not keep checking
                        // these things in next reads
                        self.child_result_shelved = None;
                    }
                }
