...
static void cursorRead(RedisModule_Reply *reply, Cursor *cursor, size_t count, bool bg) {
  ...
  if (has_spec) {
    ...
    if (!StrongRef_Get(execution_ref)) {
      ...
      // Notify the client that the query was aborted.
      RedisModule_Reply_Error(reply, "RQE_INDEX_DROPPED_DURING_CURSOR: The index was dropped while the cursor was idle");
      return;
    }
...
int RSCursorCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (RedisModule_StringToLongLong(argv[3], &cid) != REDISMODULE_OK) {
    RedisModule_ReplyWithError(ctx, "RQE_CURSOR_INVALID_ID: Bad cursor ID");
    return REDISMODULE_OK;
  }
  ...
  if (strcasecmp(cmd, "READ") == 0) {
    ...
    if (argc > 5) {
      ...
      if (strcasecmp(count_str, "count") != 0) {
        RedisModule_ReplyWithErrorFormat(ctx, "RQE_ARGUMENT_UNKNOWN: Unknown argument `%s`", count_str);
        RedisModule_EndReply(reply);
        return REDISMODULE_OK;
      }

      if (RedisModule_StringToLongLong(argv[5], &count) != REDISMODULE_OK) {
        RedisModule_ReplyWithErrorFormat(ctx, "RQE_ARGUMENT_COUNT_INVALID: Bad value for COUNT: `%s`", RedisModule_StringPtrLen(argv[5], NULL));
        RedisModule_EndReply(reply);
        return REDISMODULE_OK;
      }
    }
    ...
    if (cursor == NULL) {
      RedisModule_ReplyWithErrorFormat(ctx, "RQE_CURSOR_NOT_FOUND: Cursor not found, id: %d", cid);
      RedisModule_EndReply(reply);
      return REDISMODULE_OK;
    }
    ...
    if (rc != REDISMODULE_OK) {
      RedisModule_Reply_Error(reply, "RQE_CURSOR_DOES_NOT_EXIST: Cursor does not exist");
    } else {
    ...
    RedisModule_Reply_Error(reply, "RQE_COMMAND_SUBCOMMAND_UNKNOWN: Unknown subcommand");
  }
  RedisModule_EndReply(reply);
  return REDISMODULE_OK;
}
